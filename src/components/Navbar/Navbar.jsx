...
const Navbar = () => {
    ...
    return (
        <nav className="w-full bg-white shadow-sm py-3 px-6 flex items-center justify-between">
            ...
            {/* Middle Menu */}
            <div className="flex items-center gap-6"> <NavLink to="/" className="text-gray-700 hover:text-blue-600 font-medium">Home</NavLink>

            {/* My Profile only login */}
            {user && (
             <NavLink to="/profile" className="text-gray-700 hover:text-blue-600 font-medium">My Profile</NavLink>)}
            </div>

            {/* Right Side */}
            <div className="flex items-center gap-4">{user ? (<div className="flex items-center gap-4">
            {/* Avatar with Tooltip */}
            <div className="relative group cursor-pointer">
            <img src={user.photoURL || defaultAvatar} alt="avatar"className="w-10 h-10 rounded-full border"/>
            ...
            <p className="text-xs opacity-80">{user.email}</p></div>
            </div>

            {/* Sign Out Button */}
            <button onClick={handleSignOut}className="bg-blue-600 text-white font-semibold px-5 py-2 rounded-xl hover:bg-blue-700">Sign Out</button>
            </div>) : (<>
